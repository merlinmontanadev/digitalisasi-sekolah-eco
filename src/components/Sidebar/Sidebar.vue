...
<div  :class="`${isSidebarVisible ? '-translate-x-full' : 'translate-x-0'}`" class="fixed mt-16 sidebardw top-0 left-0 bg-white w-64 h-full shadow-xl transition-transform ">
  ...
  <div class="flex mt-3 text-center items-center justify-between">
          <router-link :to="`/admin/user-management/edit/user/${user?.user_id}`" >
            ...
            <UserIcon class="mr-2 h-5 w-5 text-white"></UserIcon>
            {{ $t("profile") }}</button>
          </router-link>
          <button @click="logout" class="w-full font-semibold text-sm mt-2 flex bg-gradient-to-r from-red-500 to-red-400 text-white px-4 py-2 rounded-r-md">
            <ArrowLeftStartOnRectangleIcon class="mr-2 h-5 w-5 text-white"></ArrowLeftStartOnRectangleIcon>
            {{ $t("logout") }}</button>
  </div>
    </div>
</div>
